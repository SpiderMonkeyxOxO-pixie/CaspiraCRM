...
export const fetchPriceBooks = createAsyncThunk("sales/priceBooks/fetchAll", async (_, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(error.response?.data?.message || "Failed to load Price Books");
  }
});
...
export const fetchPriceBook = createAsyncThunk("sales/priceBooks/fetchOne", async (id, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(error.response?.data?.message || "Failed to load the Price Book");
  }
});

export const createPriceBook = createAsyncThunk("sales/priceBooks/create", async (payload, { rejectWithValue }) => {
  try {
    const res = axiosInstance.post("/sales/price-books", payload);
    toast.promise(res, { loading: "Adding Price Book...", success: "Price Book added", error: "Failed to add Price Book" });
    const { data } = await res;
    return data.priceBook;
  } catch (error) {
    return rejectWithValue(error.response?.data || { message: "Failed to add Price Book" });
  }
});
...
export const updatePriceBook = createAsyncThunk("sales/priceBooks/update", async ({ id, changes }, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(error.response?.data || { message: "Failed to update the Price Book" });
  }
});
...
export const archivePriceBook = createAsyncThunk("sales/priceBooks/archive", async ({ id, reason }, { rejectWithValue }) => {
  ...
  try {
    const res = axiosInstance.post(`/sales/price-books/${id}/archive`, { reason });
    toast.promise(res, { loading: "Archiving...", success: "Price Book archived", error: "Failed to archive Price Book" });
    const { data } = await res;
    return data.priceBook;
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || "Failed to archive Price Book");
  }
});

export const restorePriceBook = createAsyncThunk("sales/priceBooks/restore", async (id, { rejectWithValue }) => {
  try {
    const res = axiosInstance.post(`/sales/price-books/${id}/restore`);
    toast.promise(res, { loading: "Restoring...", success: "Price Book restored", error: "Failed to restore Price Book" });
    const { data } = await res;
    return data.priceBook;
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || "Failed to restore Price Book");
  }
});

export const bulkAssignOwner = createAsyncThunk("sales/priceBooks/bulkAssign", async ({ priceBookIds, ownerId }, { rejectWithValue }) => {
  try {
    const { data } = await axiosInstance.post("/sales/price-books/bulk/assign", { priceBookIds, ownerId });
    toast.success(`${data.priceBooks.length} Price Book(s) reassigned`);
    return data.priceBooks;
  } catch (error) {
...
export const bulkChangeStatus = createAsyncThunk("sales/priceBooks/bulkStatus", async ({ priceBookIds, status }, { rejectWithValue }) => {
  try {
    const { data } = await axiosInstance.post("/sales/price-books/bulk/status", { priceBookIds, status });
    toast.success(`${data.priceBooks.length} Price Book(s) updated`);
    return data.priceBooks;
  } catch (error) {
...
export const bulkArchivePriceBooks = createAsyncThunk("sales/priceBooks/bulkArchive", async ({ priceBookIds, reason }, { rejectWithValue }) => {
  ...
  try {
    const { data } = await axiosInstance.post("/sales/price-books/bulk/archive", { priceBookIds, reason });
    toast.success(`${data.priceBooks.length} Price Book(s) archived`);
    return data.priceBooks;
  } catch (error) {
