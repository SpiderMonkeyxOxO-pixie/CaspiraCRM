...
export const createInvoice = createAsyncThunk("finance/invoices/create", async (invoiceData, { rejectWithValue }) => {
  try {
    const res = axiosInstance.post("/finance/invoices", invoiceData);
...
export const createInvoiceFromOrder = createAsyncThunk(
  ...
  async (orderId, { rejectWithValue }) => {
    try {
      const res = axiosInstance.post(`/sales/orders/${orderId}/invoice`);
      toast.promise(res, { loading: "Creating invoice...", success: "Invoice created", error: "Failed to create invoice" });
      const { data } = await res;
      return data.invoice;
    } catch (error) {
...
export const approveInvoice = createAsyncThunk("finance/invoices/approve", async ({ id, approver }, { rejectWithValue }) => {
  try {
    const { data } = await axiosInstance.put(`/finance/invoices/${id}`, { status: "Approved", approvedBy: approver });
    toast.success("Invoice approved");
    return data.invoice;
  } catch (error) {
    return rejectWithValue(error.response?.data?.message || "Failed to approve invoice");
  }
});
...
export const markInvoiceSent = createAsyncThunk("finance/invoices/markSent", async (id, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(error.response?.data?.message || "Failed to mark invoice as sent");
  }
});
...
export const voidInvoice = createAsyncThunk("finance/invoices/void", async ({ id, reason }, { rejectWithValue }) => {
  ...
  try {
    const { data } = await axiosInstance.put(`/finance/invoices/${id}`, { status: "Void", voidReason: reason });
...
export const recordPayment = createAsyncThunk(
  ...
  async ({ id, amount, method }, { rejectWithValue }) => {
    ...
    try {
      const res = axiosInstance.post(`/finance/invoices/${id}/payments`, { amount, method });
