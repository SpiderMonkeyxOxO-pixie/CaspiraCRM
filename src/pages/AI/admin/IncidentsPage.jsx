  ...
  return (
    <Modal title={i?.title || "Incident"} onClose={onClose}>
      ...
      {dialog === "regression" && <ReasonDialog title="Create a regression case" requireReason={false} confirmLabel="Add to regression suite" description="The case is added to the incident dataset (new version); restoration needs a passing run that includes it."
        fields={[{ name: "kind", label: "Kind", type: "select", options: [["deterministic", "Deterministic check"], ["copilot", "Copilot turn (fixture data)"]] }, { name: "text", label: "Input text (e.g. the injection or request that failed)", type: "textarea" }, { name: "expected", label: "Expected outcome", type: "select", options: [["flagged", "Flagged as injection"], ["deny", "Refused"]] }]}
